...
test('should display the correct headline', async () => {
    await expect(home.getHeadline()).toHaveText('Hacking aninput-driven AudioContext');
});
...
test('should go to the next slide when using the arrow key', async ({ page }) => {
    ...
    await expect(home.getSubHeadline()).toHaveText('Salut,je suis Christoph Guttandin');
});
